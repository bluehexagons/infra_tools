"""Tests for shared auto-update systemd provisioning."""
...
class TestConfigureAutoUpdateTimer(unittest.TestCase):
    def test_writes_hardened_units_and_verifies_timer(self):
        with tempfile.TemporaryDirectory() as unit_dir, patch(
            "lib.auto_update_systemd.SYSTEMD_DIR", unit_dir
        ), patch("lib.auto_update_systemd.run") as mock_run:
            mock_run.return_value = SimpleNamespace(returncode=0)

            configured = configure_auto_update_timer(
