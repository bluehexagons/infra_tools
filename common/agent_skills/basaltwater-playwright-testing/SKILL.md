---
name: basaltwater-playwright-testing
...
`~/.local/state/basaltwater/playwright-mcp` directory. Omit `filename` for
...
Use the `basaltwater-web-gateway` skill only when a task requires another
