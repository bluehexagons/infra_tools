...
def install_ruby(username: str, os_type: str, **_) -> None:
    """Install rbenv + latest Ruby version, ready to bundle install a Rails app."""
    safe_username = shlex.quote(username)
    user_home = f"/home/{username}"
...
def install_node(username: str, os_type: str, **_) -> None:
    """Install nvm + latest Node.JS + PNPM + update NPM."""
    safe_username = shlex.quote(username)
    user_home = f"/home/{username}"
    ...
    # nvm install script adds to bashrc automatically, but verify
    bashrc_path = f"{user_home}/.bashrc"
    if not file_contains(bashrc_path, "NVM_DIR"):
        nvm_init = '''
