...
`workstation_dev` adds XFCE, Firefox ESR, and the shared CLI bundle, including
Neovim. Select a graphical editor with `--editor geany` or `--editor vscode`.
Agent tools are selected individually; add T3 Code with
