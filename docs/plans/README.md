...
| Generic VM management, agent interfaces, and lightweight Git hosting | Ready for roadmap sequencing; mutation/recovery slices are dependency-gated | Unscheduled | No dedicated issue | [VM management, agent interfaces, and lightweight Git hosting](VM_MANAGEMENT_AND_LIGHTWEIGHT_GIT_HOSTING.md): replace development-era guest commands with a provider-neutral VM CLI backed by Proxmox, retain manual VPS setup, add declarative Proxmox-backed guest data disks and UUID-based mounts, keep T3 Code desktop and headless web modes explicit, add optional Nginx Basic Auth, define Samba asset/backup boundaries, and make Gogs Git LFS exposure, storage, and recovery explicit. |
