...
| One shared desktop session per machine | Proposed; implementation not started | Unscheduled | Desktop resize/reconnect validation overlaps the desktop audit | [Single-session desktop](SINGLE_SESSION_DESKTOP.md): replace independent local/RDP sessions with one lifecycle shared by humans and agents; qualify the backend, then deliver runtime, access, agent tools, and migration across desktop profiles. |
