...
| Generic VM management, agent interfaces, and lightweight Git hosting | Active; initial VM data-disk/mount slice and explicit Gogs LFS paths implemented, mutation/recovery slices dependency-gated | Unscheduled | No dedicated issue | [VM management, agent interfaces, and lightweight Git hosting](VM_MANAGEMENT_AND_LIGHTWEIGHT_GIT_HOSTING.md): next validate the storage slice on live Proxmox, then sequence provider-neutral VM observation, Gogs safety/health, and explicit T3 Code interface work without bypassing transaction and recovery gates. |
