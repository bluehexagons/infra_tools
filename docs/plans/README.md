...
| Generic VM management, agent web interfaces, and lightweight Git hosting | Ready for roadmap sequencing; mutation/recovery slices are dependency-gated | Unscheduled | No dedicated issue | [VM management, agent web interfaces, and lightweight Git hosting](VM_MANAGEMENT_AND_LIGHTWEIGHT_GIT_HOSTING.md): replace development-era guest commands with a provider-neutral VM CLI backed by Proxmox, retain manual VPS setup, add explicit and access-controlled agent web services beginning with T3 Code, and make Gogs Git LFS exposure, storage, and recovery explicit. |
