...
- Status: workspace paths, notification targets, credential usernames with ambiguous separators, deploy target hostnames, and deploy site/path specs are now validated before setup/patch or credential-management flows continue
...
4. Expand validation further across more external inputs now that workspace, notification, credential-name, deploy-target, and deploy-spec checks are in place
