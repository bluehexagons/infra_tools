...
def create_setup_argument_parser(
    ...
    parser.add_argument("--browser", choices=["brave", "firefox", "browsh", "vivaldi", "lynx"], 
                       default="brave" if for_remote else None,
                       help="Web browser to install (default: brave)")
    parser.add_argument("--flatpak", dest="use_flatpak" if not for_remote else "flatpak", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
                       help="Install desktop apps via Flatpak when available (non-containerized environments)")
    parser.add_argument("--office", dest="install_office" if not for_remote else "office", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
                       help="Install LibreOffice (desktop only)")
    
    # Development tools
    parser.add_argument("--ruby", dest="install_ruby" if not for_remote else "ruby", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
                       help="Install rbenv + latest Ruby version")
    parser.add_argument("--go", dest="install_go" if not for_remote else "go", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
                       help="Install latest Go version")
    parser.add_argument("--node", dest="install_node" if not for_remote else "node", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
                       help="Install nvm + latest Node.JS + PNPM + update NPM")
    
    # Deployment options
    parser.add_argument("--deploy", dest="deploy_specs" if not for_remote else "deploy", 
    ...
    parser.add_argument("--ssl", dest="enable_ssl" if not for_remote else "ssl", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
    ...
    parser.add_argument("--cloudflare", dest="enable_cloudflare" if not for_remote else "cloudflare", 
                       action=argparse.BooleanOptionalAction if not for_remote else "store_true", 
                       default=None if not for_remote else False,
    ...
    parser.add_argument("--share", dest="samba_shares" if not for_remote else "share", 
