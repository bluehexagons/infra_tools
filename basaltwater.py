...
"""
basaltwater - Unified entry point for infrastructure setup and management.

This script provides a unified interface to all basaltwater functionality,
...
def create_basaltwater_parser() -> Tuple[argparse.ArgumentParser, argparse.ArgumentParser, argparse.ArgumentParser]:
    """Create the main argument parser for basaltwater."""
    parser = argparse.ArgumentParser(
        prog=LAUNCHER_NAME,
        description="Basaltwater — infrastructure management, from one machine to your whole network",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=_build_basaltwater_epilog()
    )
    parser.add_argument(
        "--workspace",
    ...
    migration_parser = subparsers.add_parser("migrate", help="One-time migration from recent infra-tools installations")
    migration_parser.add_argument("--system", action="store_true", help="Migrate this host's system installation")
    migration_parser.add_argument("--installation", help="Explicit recent source directory when installed at a custom path")
    migration_action = migration_parser.add_mutually_exclusive_group()
    migration_action.add_argument("--apply", action="store_true", help="Apply the cutover; default is preview")
    migration_action.add_argument("--recover", action="store_true", help="Recover an interrupted migration")
    ...
    python_tools_parser.add_argument(
        ...
        dest="script_path",
        default=None,
        help="Absolute path to basaltwater.py (used to install the user launcher)",
    )
...
def main() -> int:
    ...
    if args.command == "migrate":
        ...
        return migrate(system=args.system, apply=args.apply, recovery=args.recover, installation=args.installation)
